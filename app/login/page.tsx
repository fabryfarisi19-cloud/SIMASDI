...
export default function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      /**
       * Login melalui NextAuth Credentials.
       *
       * Tidak lagi query tabel pengguna langsung dari browser.
       */
      ...
      /**
       * Setelah login berhasil, kita ambil session
       * untuk mengetahui role pengguna.
       */
      const sessionResponse = await fetch("/api/auth/session");

      if (!sessionResponse.ok) {
        alert("Login berhasil tetapi session belum tersedia.");
        return;
      }
       ...
       * kita boleh menyimpan data tampilan minimal.
       *
       * localStorage TIDAK LAGI menjadi sumber otorisasi.
       * Otorisasi berikutnya akan menggunakan session server.
       */
      localStorage.setItem(
        "user",
        JSON.stringify({
          id: session?.penggunaId ?? null,
          nama: session?.user?.name ?? "",
          jabatan: role ?? "",
          username: session?.username ?? nipBersih,
        })
      );
        ...
        <section className="bg-white p-8 md:p-14 flex items-center">
          <div className="w-full max-w-md mx-auto">
            ...
            <p className="mt-3 text-center text-xs text-slate-400">
              Gunakan akun backupumum.bapasjakbar@gmail.com untuk upload
              arsip ke Google Drive.
            </p>
